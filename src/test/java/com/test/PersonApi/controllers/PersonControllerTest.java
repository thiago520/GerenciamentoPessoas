package com.test.PersonApi.controllers;
...
import com.test.PersonApi.services.PersonService;
...
import static com.test.PersonApi.utils.PersonUtils.asJsonString;
...
public class PersonControllerTest {
    ...
    private static final String PEOPLE_PATH = "/api";
    ...
    void testWhenPOSTIsCalledThenAPersonShouldBeCreated() throws Exception {
        ...
        MessageResponseDTO expectedResponseMessage = createMessageResponse("Person sucessfully created with ID", 1L);

        when(personService.create(expectedPersonDTO)).thenReturn(expectedResponseMessage);

        mockMvc.perform(post(PEOPLE_PATH)
    ...
    void testWhenGETWithValidIsCalledThenAPersonShouldBeReturned() throws Exception {
        ...
        mockMvc.perform(get(PEOPLE_PATH + "/" + expectedValidId)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is(1)))
                .andExpect(jsonPath("$.name", is("Thiago")));
    }
    ...
    void testWhenGETIsCalledThenPeopleListShouldBeReturned() throws Exception {
        ...
        mockMvc.perform(get(PEOPLE_PATH)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id", is(1)))
                .andExpect(jsonPath("$[0].name", is("Thiago")));
    }
    ...
    void testWhenPUTIsCalledThenAPersonShouldBeUpdated() throws Exception {
        ...
        mockMvc.perform(put(PEOPLE_PATH + "/" + expectedValidId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(asJsonString(expectedPersonDTO)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message", is(expectedResponseMessage.getMessage())));
    ...
    void testWhenDELETEIsCalledThenAPersonShouldBeDeleted() throws Exception {
        ...
        mockMvc.perform(delete(PEOPLE_PATH + "/" + expectedValidId)
                        .contentType(MediaType.APPLICATION_JSON))
